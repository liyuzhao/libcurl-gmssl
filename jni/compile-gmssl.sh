...
SSL_PATH="$BASE_PATH/gmssl"
BUILD_PATH="$BASE_PATH/build"

checkExitCode() {
	if [ $1 -ne 0 ]; then
		echo "Error building gmssl library"
		cd $BASE_PATH
		exit $1
	fi
}
...
rm -rf $BUILD_PATH/gmssl
safeMakeDir $BUILD_PATH/gmssl
...
compile() {
	...
	# config
	safeMakeDir $BUILD_PATH/gmssl/$ABI
	checkExitCode $?
	echo "==========ARCH=${ARCH}"
	echo "==========BUILD_PATH/gmssl/ABI=${BUILD_PATH}/gmssl/${ABI}"
	echo "==========SYSROOT=${SYSROOT}"
	./Configure android no-shared --openssldir=$BUILD_PATH/gmssl/$ABI --prefix=$BUILD_PATH/gmssl/$ABI
	checkExitCode $?
	# clean
	make clean
	checkExitCode $?
